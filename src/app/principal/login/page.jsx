...
import { ArrowRight, BriefcaseBusiness, School, ShieldCheck, Sparkles } from "lucide-react";

const LOGIN_HIGHLIGHTS = [
  "Principal-level academic oversight",
  "Unified college dashboards and reports",
  "Built for fast operational decision-making",
];
      ...
      {loading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm">
          <div className="rounded-3xl border border-emerald-300/20 bg-slate-900/90 px-8 py-6 text-center shadow-2xl">
            ...
            <p className="mt-2 text-sm text-slate-300">Loading your principal dashboard.</p>
          </div>
        </div>
      )}

      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-[12%] top-20 h-48 w-48 rounded-full bg-emerald-400/12 blur-3xl" />
        <div className="absolute bottom-10 right-[8%] h-72 w-72 rounded-full bg-teal-500/10 blur-3xl" />
      </div>
          ...
          <section className="bg-white/92 p-7 sm:p-10">
            <div className="mx-auto max-w-md">
              <div className="flex items-center gap-4">
                <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-emerald-100 text-emerald-700 shadow-sm">
                  <School className="h-8 w-8" />
                </div>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">OSRA Portal</p>
                  <h2 className="mt-1 text-3xl font-black text-slate-950">Principal Login</h2>
                </div>
              </div>

              <p className="mt-4 text-sm leading-6 text-slate-600">
                Enter your principal credentials to continue to the college dashboard.
              </p>
              ...
              <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-600">
                Need a principal account?{" "}
                <Link href="/principal-registration" className="font-semibold text-emerald-700 hover:text-emerald-800 hover:underline">
                  Register here
                </Link>
              </div>
            </div>
