...
export function isAllowedAttendanceSession(session) {
  return ALLOWED_ATTENDANCE_SESSIONS.includes(normalizeAttendanceSession(session, ''))
}
...
export const attendanceRecordSchema = z.object({
  studentId: z
    .any()
    .refine(value => value != null && String(value).trim() !== '', {
...
export const attendanceRecordUpdateSchema = z.object({
  ...
  session: z
    .string()
    .optional()
    .transform(value => (value == null ? undefined : normalizeAttendanceSession(value, '')))
    .refine(value => value === undefined || isAllowedAttendanceSession(value), {
      message: 'Session must be FN or AN',
    }),
})
