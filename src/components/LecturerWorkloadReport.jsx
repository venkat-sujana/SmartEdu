            ...
            {data.map((row, i) => {
              ...
              return (
                <React.Fragment key={row.lecturer || row.name}>
                  ...
                  <tr className={`transition-colors ${rowBg} print:border-b print:border-black`}>
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-1 rounded-full bg-purple-100 px-2 py-0.5 text-xs font-semibold text-purple-700">
                        {hasSubjects ? `${subjects.length} Subject${subjects.length > 1 ? 's' : ''}` : 'N/A'}
                      </span>
                    </td>
                  ...
                  {/* Subject Detail Rows */}
                  {hasSubjects && subjects.map((sub, si) => (
                    <tr key={`${row.lecturer}-${si}`} className={`${rowBg} border-l-4 border-l-purple-400 bg-opacity-40 print:border-l-0`}>
