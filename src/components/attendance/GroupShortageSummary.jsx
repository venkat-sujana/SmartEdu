...
const handleExportPDF = () => {
  if (!shortageFilteredData.length) {
    alert("No students below 75% attendance to export.");
    return;
  }
  ...
  doc.text(
    `Attendance Below 75% - ${group} - ${year}`,
    pageWidth / 2,
    19,
    {
      align: "center",
  ...
  const tableBody = shortageFilteredData.map(
    (student, index) => {
      const {
        totalPresent,
        totalWorking,
  ...
  const safeYear = String(year || "year")
    .replace(/[^a-zA-Z0-9]+/g, "-")
    .replace(/^-|-$/g, "");
  ...
  doc.save(
    `${safeGroup}-${safeYear}-Attendance-Below-75-${date}.pdf`
  );
};
