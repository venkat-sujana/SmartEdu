  ...
  return (
    <div className="w-full overflow-x-auto bg-white rounded-2xl shadow-md p-5 border border-blue-200 my-2">
      <table className="min-w-[1080px] w-full text-xs md:text-sm border-collapse">
        <thead>
          <tr>
            <th className="p-2 border bg-blue-200 sticky left-0 z-20" rowSpan={3}> </th>
            {groups.map(group => (
              <th key={group} colSpan={4} className="p-2 border text-blue-900 font-extrabold text-center bg-blue-100">{group}</th>
            ))}
          </tr>
          <tr>
            {groups.map(group => (
              yearLabels.map(year => (
                <th key={`${group}-${year}`} colSpan={2} className="p-1 border text-blue-800 font-bold text-center bg-blue-50">{year}</th>
              ))
            ))}
          </tr>
          <tr>
            {groups.map(group => (
              yearLabels.map(year => (
                sessions.map(session => (
                  <th key={`${group}-${year}-${session}`} className="p-1 border text-blue-700 bg-blue-50 font-semibold text-center">{session}</th>
                ))
              ))
            ))}
          </tr>
        ...
        <tbody>
          {rowLabels.map((rowLabel) => (
            <tr key={rowLabel}>
              <td className="p-1 border text-right font-bold bg-blue-50 sticky left-0 z-10">{rowLabel}</td>
              {groups.map(group =>
                yearLabels.map(year =>
                  sessions.map(session => (
                    <td key={`${group}-${year}-${session}-row${rowLabel}`} className="p-1 border text-center">
                      {getCellData({ group, year: year === "I" ? "First Year" : "Second Year", session, row: rowLabel })}
                    </td>
                  ))
                )
              )}
