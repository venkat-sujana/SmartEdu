...
export default function AttendanceForm({ defaultGroup = "", returnUrl = "/lecturer/dashboard" }) {
  ...
  const handleSubmit = async () => {
    ...
    const attendanceRecords = filteredStudents.map((student) => ({
      ...
      collegeId, year, session: selectedSession
    }));
    ...
    <div className="mt-24 relative min-h-screen bg-gradient-to-br from-blue-200 via-white to-green-50 py-5 px-2">
      ...
      {fullscreenToastMessage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className={`bg-white rounded-xl px-8 py-6 text-xl font-bold shadow-xl border-2 flex flex-col items-center ${
            fullscreenToastMessage.includes("already marked") ? "text-red-700 border-red-400" : "text-green-700 border-green-400"
          }`}>
            <span className="text-3xl mb-3">{fullscreenToastMessage.includes("already marked") ? "❌" : "✅"}</span>
            <span className="mb-3">{fullscreenToastMessage}</span>
            <button className="mt-2 px-6 py-2 bg-blue-700 text-white rounded shadow hover:bg-blue-800 font-bold text-lg"
