...
export default function ExamsFormPage() {
  ...
  const handleChange = (e) => {
    ...
    if (name.startsWith('subject_')) {
      ...
      setFormData((prev) => {
        ...
        const totalMarks = subjectMarks.reduce((sum, val) => sum + (Number(val) || 0), 0);
        const percent = subjectMarks.length > 0 ? (totalMarks / subjectMarks.length) : 0;
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 bg-white shadow rounded">
      ...
      <h2 className="text-xl font-bold mb-4">Exam Entry Form</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select name="studentId" value={formData.studentId} onChange={handleChange} className="w-full border p-2 rounded">
        ...
        <div>
          ...
          <select name="academicYear" value={formData.academicYear} onChange={handleChange} className="w-full border p-2 rounded">
        ...
        <div>
          ...
          <select name="examType" value={formData.examType} onChange={handleChange} className="w-full border p-2 rounded">
            <option value="">-- Select Exam Type --</option>
            {['UNIT-1', 'UNIT-2', 'UNIT-3', 'UNIT-4', 'QUARTERLY', 'HALFYEARLY', 'PRE-PUBLIC-1', 'PRE-PUBLIC-2'].map((type) => (
        ...
        <div>
          ...
          <input type="date" name="examDate" value={formData.examDate} onChange={handleChange} className="w-full border p-2 rounded" />
        </div>

        {/* Subjects */}
        <div>
          <label className="block font-medium mb-1">Subjects & Marks</label>
          {['subject1', 'subject2', 'subject3', 'subject4', 'subject5', 'subject6'].map((subject) => (
            <input
              key={subject}
              type="number"
