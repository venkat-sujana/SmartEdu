...
export default function TimeTableAdminDashboard() {
  ...
  const createLecturer = async (e) => {
    ...
    try {
      ...
      setLecturerForm({ name: "", email: "", department: "", maxHoursPerWeek: 24, password: "" });
      fetchAll();
    } catch (err) {
  ...
  const createSubject = async (e) => {
    ...
    try {
      ...
      setSubjectForm((s) => ({ ...s, subjectName: "", subjectCode: "", lecturerId: "" }));
      fetchAll();
    } catch (err) {
      toast.error(err.message || "Failed");
    }
  ...
  const onEditSubject = async (subject) => {
    ...
    const subjectCode = window.prompt("Subject code", subject.subjectCode || "");
  ...
  const exportRows = useMemo(
    () =>
      (gridData?.slots || []).map((s) => ({
        ...
        Department: s.lecturerId?.department || "",
  ...
  const exportPdf = () => {
    ...
    doc.autoTable({
      startY: 26,
      head: [["Day", "Period", "Subject", "Code", "Lecturer", "Classroom"]],
      body: exportRows.map((r) => [r.Day, r.Period, r.Subject, r.SubjectCode, r.Lecturer, r.Classroom]),
          ...
          <div className="space-y-5">
            ...
            <div className="grid gap-4 xl:grid-cols-3">
              <form onSubmit={createLecturer} className="rounded-lg border p-3 space-y-2">
                ...
                <input className="w-full rounded border px-2 py-1.5" placeholder="Email" type="email" required value={lecturerForm.email} onChange={(e)=>setLecturerForm((s)=>({...s,email:e.target.value}))} />
                <input className="w-full rounded border px-2 py-1.5" placeholder="Department" required value={lecturerForm.department} onChange={(e)=>setLecturerForm((s)=>({...s,department:e.target.value}))} />
              ...
              <form onSubmit={createSubject} className="rounded-lg border p-3 space-y-2">
                ...
                <input className="w-full rounded border px-2 py-1.5" placeholder="Subject Code" required value={subjectForm.subjectCode} onChange={(e)=>setSubjectForm((s)=>({...s,subjectCode:e.target.value.toUpperCase()}))} />
                    ...
                    <tbody>
                      {lecturers.map((l) => (
                        <tr key={l._id} className="border-t">
                          ...
                          <td className="px-2 py-2">{l.department}</td>
                      ...
                      {lecturers.length === 0 && !loading && (
                        <tr>
                          <td className="px-2 py-3 text-center text-slate-500" colSpan={5}>
                            No lecturers found
                          </td>
                        </tr>
                      )}
