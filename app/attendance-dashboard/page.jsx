...
export default function AttendanceDashboardHome() {
  ...
  const { data: todayData } = useSWR('/api/attendance/today-absentees', fetcher)
  const { data: dailySummaryData } = useSWR(dailySummaryUrl, fetcher)

  const { data: todayBreakdown } = useSWR(
    collegeId ? `/api/attendance/today-breakdown?collegeId=${encodeURIComponent(collegeId)}` : null,
    fetcher
  )
