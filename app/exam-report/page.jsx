...
export default function ExamReportPage() {
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-7xl bg-gradient-to-br from-blue-50 via-white to-green-50 p-1 sm:p-4 font-sans">
      <Toaster />
      ...
      {/* Main Title */}
      <h1 className="mb-6 text-center text-xl sm:text-2xl md:text-4xl font-extrabold tracking-tight text-blue-900">
...
<div className="mb-6 flex flex-wrap justify-center gap-2 sm:gap-4">
  <div className="min-w-[80px] sm:min-w-[120px] rounded-xl border-2 border-green-200 bg-green-50 px-3 py-2 sm:px-8 sm:py-5 text-center shadow-lg">
    <div className="mb-1 text-lg sm:text-3xl">✅</div>
    <div className="text-lg sm:text-2xl font-bold text-green-700">{passCount}</div>
    <div className="text-xs sm:text-sm font-semibold text-green-800">Passed</div>
  </div>
  <div className="min-w-[80px] sm:min-w-[120px] rounded-xl border-2 border-red-200 bg-red-50 px-3 py-2 sm:px-8 sm:py-5 text-center shadow-lg">
    <div className="mb-1 text-lg sm:text-3xl">❌</div>
    <div className="text-lg sm:text-2xl font-bold text-red-700">{failCount}</div>
    <div className="text-xs sm:text-sm font-semibold text-red-800">Failed</div>
  </div>
  <div className="min-w-[80px] sm:min-w-[120px] rounded-xl border-2 border-blue-200 bg-blue-50 px-3 py-2 sm:px-8 sm:py-5 text-center shadow-lg">
    <div className="mb-1 text-lg sm:text-3xl">📈</div>
    <div className="text-lg sm:text-2xl font-bold text-blue-700">{passPercentage}%</div>
    <div className="text-xs sm:text-sm font-semibold text-blue-800">Pass Rate</div>
  </div>
</div>
      ...
      <div className="mb-6 flex flex-wrap justify-center gap-2 sm:gap-4">
        <Link href="/student-table">
          <button className="rounded-xl bg-cyan-600 px-3 py-2 sm:px-6 sm:py-3 font-semibold text-white shadow-lg transition hover:bg-cyan-700 text-xs sm:text-base">
            👥 Student Table
          </button>
        </Link>
        ...
        <button
          ...
          className="rounded-xl bg-cyan-600 px-3 py-2 sm:px-6 sm:py-3 font-semibold text-white shadow-lg transition hover:bg-cyan-700 text-xs sm:text-base"
        >
          🖨️ Print Report
        </button>
      </div>
      ...
      {/* Filters */}
      <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
        <input
          type="text"
          placeholder="Search Student Name"
          value={filters.studentName}
          onChange={e => setFilters({ ...filters, studentName: e.target.value })}
          className="rounded-xl border-2 border-blue-400 px-2 py-2 sm:px-4 sm:py-3 text-sm sm:text-base transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-400"
        />
        <select
...
<div className="min-h-screen bg-gray-50 px-2 sm:px-6 py-4" id="print-section">
  {Object.keys(examYearReports).length === 0 ? (
    ...
    Object.entries(examYearReports).map(([exam, years]) => (
      <section key={exam} className="mb-12">
        {/* Exam Type Header */}
        <div className="mb-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center text-2xl font-bold text-indigo-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {Object.entries(years).map(([year, rows]) => (
            <div key={year} className="rounded-3xl border-2 border-blue-100 bg-white shadow-xl p-4 sm:p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[360px] sm:min-w-[400px] border-collapse text-xs sm:text-sm">
                  <thead>
                    <tr className="bg-gradient-to-r from-blue-200 via-indigo-200 to-purple-200 text-blue-900">
                      ...
                      {(rows[0]?.stream && ['M&AT', 'CET', 'MLT'].includes(rows[0].stream)
                        ...
                        <th key={i} className="border px-2 py-2 text-center text-[11px] sm:text-xs sm:py-3">
                          {col}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {rows.length === 0 ? (
                      ...
                      rows.map((report, idx) => {
                        ...
                        return (
                          <tr key={idx} className={`text-center ${rowClass} border-b transition hover:bg-yellow-50`}>
